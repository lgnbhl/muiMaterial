...
format:
  html:
    minimal: true
vignette: >
  %\VignetteIndexEntry{Using a router}
...
To use pages with **muiMaterial**, you might need to use a router service. This document shows examples using the [reactRouter](https://github.com/lgnbhl/reactRouter) and [shiny.router](https://appsilon.github.io/shiny.router/) R packages.

### Examples using {reactRouter}

[reactRouter](https://github.com/lgnbhl/reactRouter) wraps React Router v7. The entry point is `RouterProvider(router = ...)`, where the router is built with `createHashRouter()`. Routes are declared with nested `Route()` calls and the active child route is rendered wherever you place `Outlet()` in the parent's `element`. The hash strategy (`createHashRouter()`) works in Shiny, Quarto and from a local `file://` page.
...
An example with [reactRouter](https://github.com/lgnbhl/reactRouter) in an R Shiny app (with server). Each route's `element` holds a `uiOutput()` that the server fills with `renderUI()`; client-side navigation mounts the matched route and Shiny renders its content automatically:
