...
For full control over the input, pass `renderInput` as a `JS()` callback. This
takes precedence over both child elements and `inputProps`.
...
Use `renderOption` with a data frame to display additional information below each option label.

**Note**: Like `groupBy` and `getOptionLabel`, define `renderOption` as a static JavaScript function. Never pass user input directly into `JS()`.
...
Set an initial value using the `value` argument. The default is `NULL` for
single selection and `[]` (an empty list) for `multiple = TRUE`.
...
The default should be in a `list()` for `multiple = TRUE`.
