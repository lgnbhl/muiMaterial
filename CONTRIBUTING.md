...
include a minimal reproducible example (a small `shinyApp(...)` block
is ideal), the output of `sessionInfo()`, and the package versions of
`muiMaterial`, `shiny` and `shiny.react`.
...
The R side has no compile step. Install dev dependencies in R and
build the package:
...
The minified bundle shipped in `inst/www/muiMaterial/` is produced
from sources in `js/`. To rebuild it:
...
`inst/www/muiMaterial/`. The webpack pipeline pins exact MUI versions
(`js/package.json`) and emits third-party license attributions to
...
- `Component.triggerId()` — overlay components bound to an existing
  DOM element by id, with no server logic required.

See `?muiMaterial` for the full rationale.

## Style

- Follow the existing code style; the project uses base R
  conventions plus `checkmate` for argument validation.
...
- Update `NEWS.md` under the unreleased section at the top of the
  file.
- Make sure `devtools::check()` is clean (0 errors, 0 warnings, at
  most the expected installed-size NOTE).
