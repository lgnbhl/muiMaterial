# Helpers: reach the reactElement payload created by the .static wrappers.
# asProps wraps the actual props one level deep under `props$value`.
static_payload <- function(tag) {
  environment(tag[["children"]][[2]])[["data"]]
}
static_props <- function(tag) {
  static_payload(tag)[["props"]][["value"]]
}
...
test_that("TabContext.static() returns a muiMaterial shiny.tag with the right name", {
  ...
  expect_equal(static_payload(tag)[["name"]], "MuiStaticTabContext")
})
...
test_that("uncontrolled mode forwards defaultValue and leaves value NULL", {
  props <- static_props(TabContext.static(defaultValue = "one"))
...
test_that("TabList.static() returns a muiMaterial shiny.tag with the right name", {
  tag <- TabList.static()
  expect_true(inherits(tag, "muiMaterial"))
  expect_equal(static_payload(tag)[["name"]], "MuiStaticTabList")
})
