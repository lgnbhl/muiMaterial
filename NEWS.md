...
- the JS build pins Babel's `@babel/preset-react` to the production
  ...
  `jsxDEV` calls, which the production React served by shiny.react does not
  provide (blank pages). This regression never reached a CRAN release.
...
- the pkgdown website navbar now links to the `llms.txt` file (LLM-friendly
  documentation).
